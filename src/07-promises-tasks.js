...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise((answerYes, answerNo) => {
    if (typeof isPositiveAnswer === 'boolean') {
      if (isPositiveAnswer) {
        answerYes('Hooray!!! She said "Yes"!');
      } else {
        answerYes('Oh no, she said "No".');
      }
    } else {
      answerNo(new Error('Wrong parameter is passed! Ask her again.'));
    }
  });
}
...
function chainPromises(array, action) {
  const arrPromise = [];
  array.map((item) => item.then((elem) => arrPromise.push(elem)));
  return new Promise((resolve) => {
    resolve(Promise.resolve().then(() => arrPromise.reduce(action)));
  });
}
