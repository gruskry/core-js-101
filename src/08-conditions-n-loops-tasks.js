...
function findFirstSingleChar(str) {
  ...
  str.split('').forEach((char) => {
    ...
    arr.has(char) ? arr.set(char, arr.get(char) + 1) : arr.set(char, 1);
  });
  const filteredArr = Array.from(arr.entries()).filter((char) => char[1] === 1);
  return filteredArr.length > 0 ? filteredArr[0][0] : null;
}
/**
...
function getDigitalRoot(num) {
  ...
  return getDigitalRoot(num.toString().split('').reduce((res, val) => Number(res) + Number(val)));
}
...
function getCommonDirectoryPath(pathes) {
  const directory = `${pathes.reduce((a, b) => b.split('/').filter((part) => a.includes(part))).join('/')}`;
  if (!directory) {
    if (pathes.every((c) => c[0] === '/')) {
      return '/';
    }
    return '';
  }
...
function getMatrixProduct(m1, m2) {
  const res = Array.from(Array(m1.length), () => Array(m2[0].length));
